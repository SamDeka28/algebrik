...
const KNOWN_RESOURCE_CENTER_BLOG_SLUGS: string[] = [
...
async function fetchAllBlogSlugsFromStrapi(): Promise<string[]> {
  const slugs: string[] = [];
  let page = 1;
  const pageSize = 100;
  try {
    ...
    console.error('generateStaticParams: paginated Strapi fetch failed:', error);
  }
  return slugs;
}

function uniqueSlugParams(slugs: string[]): { slug: string }[] {
